...
void *TrainModelThread(void *id) {
  ...
  while (1) {
    ...
    // A random integer in [0, 'window'-1]
    ...
    if (cbow) { //train the CBOW ARCHITECTURE
      ...
      // Accumulating the word vectors for all of the context words.
      for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
        
        c = sentence_position - window + a;// Converting the window offset 'a' to index 'c' in the sentence array.
          
        // Check if 'c' is in the sentence bound
        if (c < 0) continue;
        if (c >= sentence_length) continue;

        last_word = sen[c];// Get the index of the context word
          
        if (last_word == -1) continue;// Check if the word exists in vocabulary
          
        for (c = 0; c < layer1_size; c++) neu1[c] += syn0[c + last_word * layer1_size];
        cw++;// Counting the number of context words
      }
      
      // This part will be skipped if there is no context word
      if (cw) {

        // Calculating the avergae of the context word vectors
        ...
        if (hs) for (d = 0; d < vocab[word].codelen; d++) {
          ...
          // Sigmoid activation to the current output neuron.
        ...
        // NEGATIVE SAMPLING
        /* Performing backpropagation for every word in the vocabulary is computationally expensive, so we
        perform it for positive samples and a few negative samples. In the function 'InitUnigramTable()' , 
        'unigram' distribution is generated, based on that the 'Negative' samples are selected. 
        */
        if (negative > 0) for (d = 0; d < negative + 1; d++) {
          
          // We train the 'Positive' samples in the 1st iteration.
          if (d == 0) {
            target = word;
            label = 1; // Mark this as a 'Positive' sample

          // We train the 'Negative' samples in other iterations.  
          } else {
            // Select a random word as 'Negative' sample
            next_random = next_random * (unsigned long long)25214903917 + 11;
            target = table[(next_random >> 16) % table_size]; // Index of the 'Negative' word in vocabulary 
            if (target == 0) target = next_random % (vocab_size - 1) + 1;
            
            // If this is a 'Positive' sample, don't pick it as 'Negative' sample
            if (target == word) continue;
            label = 0; // Mark this as a 'Negative' sample
          }
          
          // Index of target word (Positive/Negative) in the output layer
          l2 = target * layer1_size;

          /*Calculate the dot product between the average of the context word vectors.
          and the output weights for the target word*/
          f = 0;
          for (c = 0; c < layer1_size; c++) f += neu1[c] * syn1neg[c + l2];
          
          
          /* Calculate the output of the network for the training pair and 
          ...
          */
          ...
          // Summing up the gradients over all 'Negative' samples
        ...
        // hidden -> in
        // The error is backpropagated here.
        for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
